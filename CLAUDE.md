...
- **FormWidgets**: which template renders a field type. `add('colour', 'myapp:widget/colour')` adds one; adding an existing name replaces it. The seven built-ins are registered by its constructor and live one per file in `views/widget/`. **This replaced `Form::VIEW_INPUT`** — an `if/elseif` chain whose only override was a constant, so the first application to add a field type closed the door behind it. `VIEW_ERRORS` and `VIEW_FIELD` remain constants, because there is one of each. An unregistered type renders an HTML comment naming it and logs what *is* registered; it used to render an empty string and say nothing anywhere.
